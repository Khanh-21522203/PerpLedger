...
//
// IMPORTANT: Must match in-memory BalanceTracker semantics:
//   debit account:  balance += amount  (receives value)
//   credit account: balance -= amount  (gives value)
func (pw *ProjectionWorker) updateBalanceProjection(ctx context.Context, tx *sql.Tx, j JournalEntry) error {
	// Debit account: increase balance (receives value)
	if _, err := tx.ExecContext(ctx, `
	...
	// Credit account: decrease balance (gives value)
	if _, err := tx.ExecContext(ctx, `
		INSERT INTO projections.balances (account_path, asset_id, balance, last_sequence)
		VALUES ($1, $2, -$3, $4)
		ON CONFLICT (account_path, asset_id) 
...
func RebuildProjections(ctx context.Context, db *sql.DB) error {
	...
	// Debit accounts: balance += amount (receives value)
	...
	// Credit accounts: balance -= amount (gives value)
	_, err = db.ExecContext(ctx, `
		INSERT INTO projections.balances (account_path, asset_id, balance, last_sequence)
		SELECT 
